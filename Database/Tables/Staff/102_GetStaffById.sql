-- =============================================
-- Stored Procedure: 102_GetStaffById
-- Versión 102: Asignación por escuela (SchoolStaff) en lugar de SiteStaff.
-- =============================================

CREATE OR ALTER PROCEDURE [dbo].[102_GetStaffById]
    @id INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT
        ...
        sch.AgencyId AS SchoolAgencyId,
        ssch.AssignmentTypeId,
        ...
        ssch.StartDate,
        ssch.EndDate,
        ssch.Comments AS AssignmentComments
    FROM Staff s
        ...
        LEFT JOIN SchoolStaff ssch ON s.Id = ssch.StaffId AND ssch.IsActive = 1
        LEFT JOIN School sch ON ssch.SchoolId = sch.Id
        LEFT JOIN OptionSelection os_assignment ON ssch.AssignmentTypeId = os_assignment.Id
    ...
    SELECT
        ...
        os.DisplayOrder,
        os.IsDefaultValue
    FROM StaffSalaryOrigin sso
        INNER JOIN OptionSelection os ON sso.OptionSelectionId = os.Id
    WHERE sso.StaffId = @id
        AND sso.IsActive = 1
