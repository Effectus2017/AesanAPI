-- =============================================
-- Stored Procedure: 114_GetAgencyByIdAndUserId
-- Fecha: 2026-03-18
-- Descripción: Obtiene los datos de una agencia por su ID y el ID del usuario con lógica de acceso.
--              Reemplaza 113_GetAgencyByIdAndUserId. Añade acceso por UserProgram (evaluadores y
--              usuarios que ven agencias por programa), alineado con 120_GetAgencies.
--              El 2º result set devuelve los programas de la agencia (AgencyProgram para @agencyId),
--              sin filtrar por AgencyUsers, para que evaluadores reciban agency.programs.
-- =============================================

CREATE OR ALTER PROCEDURE [114_GetAgencyByIdAndUserId]
    @agencyId INT,
    @userId NVARCHAR(450)
AS
BEGIN
    ...
    -- =============================================
    -- LÓGICA DE ACCESO: Verificar acceso del usuario
    -- =============================================
    ...
    ELSE
    BEGIN
        -- Verificar si el usuario tiene acceso a esta agencia específica (AgencyUsers)
        IF EXISTS (
        ...
        -- Si no tiene acceso por AgencyUsers, verificar por programa (Evaluadores y otros: UserProgram + AgencyProgram, igual que 120_GetAgencies)
        IF @hasAccess = 0
        BEGIN
            IF EXISTS (
                SELECT 1
                FROM UserProgram up
                INNER JOIN AgencyProgram ap ON up.ProgramId = ap.ProgramId AND ap.AgencyId = @agencyId AND ap.IsActive = 1
    ...
    -- Segunda consulta: Programas de la agencia (para cualquier usuario que ya pasó la verificación de acceso)
    SELECT DISTINCT
        p.Id,
        p.Name,
        p.NameEN,
...
--EXEC [114_GetAgencyByIdAndUserId] @agencyId = 8, @userId = '77BC70EE-D0A8-4904-99D6-C92CB3D632DA';
