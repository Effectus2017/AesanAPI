-- =============================================
-- Stored Procedure: 121_GetAgencies
-- Fecha: 2026-03-18
-- Descripción: Versión para aesan-portal/sponsor-evaluation. Requiere @userId y relaciona
--              el usuario con: (1) asignación a la agencia (AgencyUsers), (2) acceso por
--              programa (UserProgram + AgencyProgram), (3) cuando @isPropietary=0 exige
--              que el usuario sea NUTRE (RoleAssignmentCategory = N'NUTRE'), salvo que
--              sea administrador (super_administrator/administrator), que siempre ve todo.
--              No permite @userId IS NULL para evitar devolver todas las agencias.
--              Misma estructura y result sets que 120_GetAgencies.
-- =============================================

CREATE OR ALTER PROCEDURE [121_GetAgencies]
    @take INT = 10000000,
    @skip INT = 0,
...
BEGIN
    ...
    WHERE (@isPropietary IS NULL OR a.IsPropietary = @isPropietary)
        ...
        AND (
            ...
            OR EXISTS (
                SELECT 1
                FROM UserProgram up
                INNER JOIN AgencyProgram ap_ev ON up.ProgramId = ap_ev.ProgramId AND ap_ev.AgencyId = a.Id
                WHERE up.UserId = @userId
                    AND up.IsActive = 1
                    AND ap_ev.IsActive = 1
            )
        )
        AND (
            @isPropietary IS NULL
            OR @isPropietary = 1
            OR EXISTS (
                SELECT 1
                FROM AspNetUserRoles ur_adm
                INNER JOIN AspNetRoles r_adm ON ur_adm.RoleId = r_adm.Id
                WHERE ur_adm.UserId = @userId
                    AND r_adm.Name IN ('super_administrator', 'administrator')
            )
            OR EXISTS (
                SELECT 1
                FROM AspNetUserRoles ur_n
                INNER JOIN RoleAssignmentCategory rac ON ur_n.RoleId = rac.RoleId AND rac.AssignmentCategory = N'NUTRE'
                WHERE ur_n.UserId = @userId
            )
        )
        AND (
            @alls = 1
            OR (
                ...
                AND (
                    own.UserId = @userId
                    OR EXISTS (SELECT 1 FROM AgencyUsers au2 WHERE au2.UserId = @userId AND au2.AgencyId = a.Id AND au2.IsActive = 1)
                    OR EXISTS (
                        SELECT 1
                        FROM UserProgram up2
                        INNER JOIN AgencyProgram ap_ev2 ON up2.ProgramId = ap_ev2.ProgramId AND ap_ev2.AgencyId = a.Id
                        WHERE up2.UserId = @userId
                            AND up2.IsActive = 1
                            AND ap_ev2.IsActive = 1
                    )
                )
            )
        )
    ...
    -- Usuarios asignados a la agencia
    WITH
        AgencyProgramsCTE
        AS
        (
