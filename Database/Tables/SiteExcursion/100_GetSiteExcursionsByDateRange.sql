-- =============================================
-- Stored Procedure: 100_GetSiteExcursionsByDateRange
-- Descripción: Obtiene excursiones de un sitio por rango de fechas
-- Fecha: 2025-03-14
-- Versión: 1.0
-- =============================================

CREATE OR ALTER PROCEDURE [dbo].[100_GetSiteExcursionsByDateRange]
    @siteId INT,
    @startDate DATE,
    @endDate DATE,
...
BEGIN
    ...
    -- Obtener excursiones del sitio en el rango de fechas
    SELECT
    ...
    WHERE se.SiteId = @siteId
        AND se.ExcursionDate >= @startDate
        AND se.ExcursionDate <= @endDate
        AND (@includeInactive = 1 OR se.IsActive = 1)
    ORDER BY se.ExcursionDate ASC, se.CreatedAt DESC;
