-- =============================================
-- Migración automática de procedimiento almacenado
-- Fecha: 2025-03-07 19:52:33
-- Archivo original: /Volumes/Mac/Proyectos/AESAN/Proyecto/Api/Database/StoredProcedures/Agency/103_UpdateInsertAgency.sql
-- =============================================

-- Procedimiento para insertar una agencia con las nuevas propiedades
CREATE OR ALTER PROCEDURE [103_InsertAgency]
    @Name NVARCHAR(255),
    @AgencyStatusId INT,
    @SdrNumber INT,
    @UieNumber INT,
    @EinNumber INT,
    @Address NVARCHAR(255),
    @ZipCode INT,
    @Phone NVARCHAR(20),
    @Email NVARCHAR(255),
    @CityId INT,
    @RegionId INT,
    @PostalAddress NVARCHAR(255),
    @PostalZipCode INT,
    @PostalCityId INT,
    @PostalRegionId INT,
    @Latitude FLOAT,
    @Longitude FLOAT,
    @NonProfit BIT,
    @FederalFundsDenied BIT,
    @StateFundsDenied BIT,
    @OrganizedAthleticPrograms BIT,
    @AtRiskService BIT,
    @ServiceTime DATETIME,           -- Nueva propiedad
    @TaxExemptionStatus INT,         -- Nueva propiedad
    @TaxExemptionType INT,           -- Nueva propiedad
    @RejectionJustification NVARCHAR(MAX) = NULL,
    @ImageURL NVARCHAR(MAX) = NULL,
    @Comment NVARCHAR(MAX) = NULL,
    @AppointmentCoordinated BIT = NULL,
    @AppointmentDate DATETIME = NULL,
    @IsListable BIT = 1,
    @AgencyCode NVARCHAR(50),
    @Id INT OUTPUT
AS
BEGIN
    SET NOCOUNT ON;

    INSERT INTO Agency (
        Name, AgencyStatusId, SdrNumber, UieNumber, EinNumber,
        Address, ZipCode, Phone, Email, CityId, RegionId,
        PostalAddress, PostalZipCode, PostalCityId, PostalRegionId,
        Latitude, Longitude, NonProfit, FederalFundsDenied, StateFundsDenied,
        OrganizedAthleticPrograms, AtRiskService, 
        ServiceTime, TaxExemptionStatus, TaxExemptionType,  -- Nuevas propiedades
        RejectionJustification, ImageURL, Comment,
        AppointmentCoordinated, AppointmentDate, IsListable, AgencyCode
    )
    VALUES (
        @Name, @AgencyStatusId, @SdrNumber, @UieNumber, @EinNumber,
        @Address, @ZipCode, @Phone, @Email, @CityId, @RegionId,
        @PostalAddress, @PostalZipCode, @PostalCityId, @PostalRegionId,
        @Latitude, @Longitude, @NonProfit, @FederalFundsDenied, @StateFundsDenied,
        @OrganizedAthleticPrograms, @AtRiskService, 
        @ServiceTime, @TaxExemptionStatus, @TaxExemptionType,  -- Nuevas propiedades
        @RejectionJustification, @ImageURL, @Comment,
        @AppointmentCoordinated, @AppointmentDate, @IsListable, @AgencyCode
    );

    SET @Id = SCOPE_IDENTITY();
    RETURN @Id;
END;
GO 
GO
