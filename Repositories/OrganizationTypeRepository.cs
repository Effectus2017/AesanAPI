...
public class OrganizationTypeRepository(DapperContext context, ILogger<OrganizationTypeRepository> logger) : IOrganizationTypeRepository
{
    private readonly DapperContext _context = context ?? throw new ArgumentNullException(nameof(context));
    private readonly ILogger<OrganizationTypeRepository> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    public async Task<dynamic> GetOrganizationTypeById(int id)
    {
        try
        {
            _logger.LogInformation("Obteniendo tipo de organización por ID: {Id}", id);
            using IDbConnection db = _context.CreateConnection();
            var parameters = new DynamicParameters();
            parameters.Add("@id", id, DbType.Int32);
            var result = await db.QueryMultipleAsync("100_GetOrganizationTypeById", parameters, commandType: CommandType.StoredProcedure);
            var data = await result.ReadSingleAsync<DTOOrganizationType>();
            return data;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error al obtener el tipo de organización por ID: {Id}", id);
            throw;
        }
    }
    ...
    public async Task<dynamic> GetAllOrganizationTypes(int take, int skip, string name, bool alls)
    {
        try
        {
            _logger.LogInformation("Obteniendo todos los tipos de organización");
            using IDbConnection db = _context.CreateConnection();
            var parameters = new DynamicParameters();
    ...
    public async Task<bool> InsertOrganizationType(DTOOrganizationType organizationType)
    {
        try
        {
            _logger.LogInformation("Insertando tipo de organización: {Name}", organizationType.Name);
            using IDbConnection db = _context.CreateConnection();
            var parameters = new DynamicParameters();
        ...
        {
            _logger.LogError(ex, "Error al insertar el tipo de organización: {Name}", organizationType.Name);
            throw;
        }
    }
    ...
    public async Task<bool> UpdateOrganizationType(DTOOrganizationType organizationType)
    {
        try
        {
            _logger.LogInformation("Actualizando tipo de organización: {Name}", organizationType.Name);
            using IDbConnection db = _context.CreateConnection();
            var parameters = new DynamicParameters();
            ...
            await db.ExecuteAsync("100_UpdateOrganizationType", parameters, commandType: CommandType.StoredProcedure);
            var rowsAffected = parameters.Get<int>("@rowsAffected");
            return rowsAffected > 0;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error al actualizar el tipo de organización: {Name}", organizationType.Name);
            throw;
        }
    }
    ...
    public async Task<bool> DeleteOrganizationType(int id)
    {
        try
        {
            _logger.LogInformation("Eliminando tipo de organización: {Id}", id);
            using IDbConnection db = _context.CreateConnection();
            var parameters = new DynamicParameters();
            parameters.Add("@id", id, DbType.Int32);
            parameters.Add("@rowsAffected", dbType: DbType.Int32, direction: ParameterDirection.ReturnValue);

            await db.ExecuteAsync("100_DeleteOrganizationType", parameters, commandType: CommandType.StoredProcedure);
            var rowsAffected = parameters.Get<int>("@rowsAffected");
            return rowsAffected > 0;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error al eliminar el tipo de organización: {Id}", id);
            throw;
        }
    }
}