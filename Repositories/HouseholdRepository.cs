...
namespace Api.Repositories
{
    public class HouseholdRepository(DapperContext context, ILoggingService logger, IMemoryCache cache, IOptions<ApplicationSettings> appSettings) : IHouseholdRepository
    {
        ...
        /// <summary>
        /// Obtiene un HouseholdMemberIncome por su Id
        /// </summary>
        /// <param name="id">Id del HouseholdMemberIncome a obtener</param>
        /// <returns>DTOHouseholdMemberIncome con la información del ingreso</returns>
        public async Task<dynamic> GetHouseholdMemberIncomeById(int id)
        {
            try
            {
                using IDbConnection dbConnection = _context.CreateConnection();
                var parameters = new DynamicParameters();
                parameters.Add("@Id", id, DbType.Int32, ParameterDirection.Input);
                var result = await dbConnection.QueryFirstOrDefaultAsync<DTOHouseholdMemberIncome>("101_GetHouseholdMemberIncomeById", parameters, commandType: CommandType.StoredProcedure);
                return result;
            }
            catch (Exception ex)
            {
                await _logger.LogError(ex, $"Error al obtener HouseholdMemberIncome por Id: {id}");
                throw;
            }
        }

        /// <summary>
        /// Obtiene una lista paginada de HouseholdMemberIncomes para un HouseholdMember
        /// </summary>
        /// <param name="memberId">Id del HouseholdMember</param>
        /// <param name="take">Cantidad de registros a tomar</param>
        /// <param name="skip">Cantidad de registros a saltar</param>
        public async Task<dynamic> GetHouseholdMemberIncomes(int take, int skip, int memberId)
        {
            try
            {
                using IDbConnection dbConnection = _context.CreateConnection();
                ...
                parameters.Add("@skip", skip, DbType.Int32, ParameterDirection.Input);
                parameters.Add("@memberId", memberId, DbType.Int32, ParameterDirection.Input);
                var result = await dbConnection.QueryAsync<DTOHouseholdMemberIncome>("101_GetHouseholdMemberIncomes", parameters, commandType: CommandType.StoredProcedure);
                return result;
            }
            catch (Exception ex)
            {
                await _logger.LogError(ex, $"Error al obtener HouseholdMemberIncomes para MemberId: {memberId}");
                throw;
            }
        }

        /// <summary>
        /// Inserta un nuevo HouseholdMemberIncome
        /// </summary>
        /// <param name="request">Datos del ingreso a insertar</param>
        /// <returns>Id del ingreso insertado</returns>
        public async Task<int> InsertHouseholdMemberIncome(HouseholdMemberIncomeRequest request)
        {
            try
            {
                using IDbConnection dbConnection = _context.CreateConnection();
                var parameters = new DynamicParameters();
                parameters.Add("@MemberId", request.MemberId, DbType.Int32, ParameterDirection.Input);
                parameters.Add("@IncomeType", request.IncomeType, DbType.String, ParameterDirection.Input);
                parameters.Add("@Amount", request.Amount, DbType.Decimal, ParameterDirection.Input);
                parameters.Add("@Frequency", request.Frequency, DbType.String, ParameterDirection.Input);
                parameters.Add("@Id", dbType: DbType.Int32, direction: ParameterDirection.Output);
                await dbConnection.ExecuteAsync("101_InsertHouseholdMemberIncome", parameters, commandType: CommandType.StoredProcedure);
                return parameters.Get<int>("@Id");
            }
            catch (Exception ex)
            {
                await _logger.LogError(ex, "Error al insertar HouseholdMemberIncome");
                throw;
            }
        }

        /// <summary>
        /// Actualiza un HouseholdMemberIncome existente
        /// </summary>
        /// <param name="request">Datos del ingreso a actualizar</param>
        /// <returns>True si la actualización fue exitosa</returns>
        public async Task<bool> UpdateHouseholdMemberIncome(HouseholdMemberIncomeRequest request)
        {
            try
            {
                using IDbConnection dbConnection = _context.CreateConnection();
                ...
                parameters.Add("@Amount", request.Amount, DbType.Decimal, ParameterDirection.Input);
                parameters.Add("@Frequency", request.Frequency, DbType.String, ParameterDirection.Input);
                var rows = await dbConnection.ExecuteAsync("101_UpdateHouseholdMemberIncome", parameters, commandType: CommandType.StoredProcedure);
                return rows > 0;
            }
            catch (Exception ex)
            {
                await _logger.LogError(ex, "Error al actualizar HouseholdMemberIncome");
                throw;
            }
        }

        /// <summary>
        /// Elimina un HouseholdMemberIncome por su Id
        /// </summary>
        /// <param name="id">Id del HouseholdMemberIncome a eliminar</param>
        /// <returns>True si la eliminación fue exitosa</returns>
        public async Task<bool> DeleteHouseholdMemberIncome(int id)
        {
            try
            {
                using IDbConnection dbConnection = _context.CreateConnection();
                var parameters = new DynamicParameters();
                parameters.Add("@Id", id, DbType.Int32, ParameterDirection.Input);
                var rows = await dbConnection.ExecuteAsync("101_DeleteHouseholdMemberIncome", parameters, commandType: CommandType.StoredProcedure);
                return rows > 0;
            }
            catch (Exception ex)
            {
                await _logger.LogError(ex, $"Error al eliminar HouseholdMemberIncome Id: {id}");
                throw;
            }
        }
    }
