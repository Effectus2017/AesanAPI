...
public class SiteController(ILogger<SiteController> logger, IUnitOfWork unitOfWork) : Controller
{
    ...
    [SwaggerOperation(Summary = "Verifica si existe un sitio principal", Description = "Devuelve true si existe un sitio principal, false en caso contrario.")]
    public async Task<ActionResult<bool>> HasMainSite()
    {
        try
        {
            var result = await _unitOfWork.SiteRepository.HasMainSite();

            if (!result)
            {
                return NotFound("No se encontró ningún sitio principal");
            }
        ...
        {
            _logger.LogError(ex, "Error al verificar si existe un sitio principal: {Message}", ex.Message);
            return StatusCode(500, ex.Message);
        }
    }
